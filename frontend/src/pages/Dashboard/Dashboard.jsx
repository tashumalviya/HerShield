...
const handleDelete = async () => {
  ...
  try {
    ...
    const res = await axios.delete(
      `https://hershield-f3bj.onrender.com/api/auth/delete/${user.id}`
    );
  ...
  const handleSOS = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          await fetch("https://hershield-f3bj.onrender.com/api/tracking/start", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_id: user.id, latitude: lat, longitude: lng }),
          });

          const response = await fetch("https://hershield-f3bj.onrender.com/api/sos", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ user_id: user.id, latitude: lat, longitude: lng }),
          });
          ...
          window.liveTracking = setInterval(() => {
            navigator.geolocation.getCurrentPosition(
              async (pos) => {
                await fetch("https://hershield-f3bj.onrender.com/api/tracking/update", {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({
                    user_id: user.id,
